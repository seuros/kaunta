-- Migration 000012: Add Entry/Exit Page Tracking
-- Tracks first page (entry) and last page (exit) for each session

-- ============================================================================
-- 1. ADD COLUMNS TO session TABLE
-- ============================================================================

ALTER TABLE session ADD COLUMN IF NOT EXISTS entry_page VARCHAR(500);
ALTER TABLE session ADD COLUMN IF NOT EXISTS exit_page VARCHAR(500);

-- ============================================================================
-- 2. CREATE INDEXES FOR ENTRY/EXIT PAGE QUERIES
-- ============================================================================

CREATE INDEX IF NOT EXISTS idx_session_entry_page
    ON session(website_id, entry_page)
    WHERE entry_page IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_session_exit_page
    ON session(website_id, exit_page)
    WHERE exit_page IS NOT NULL;

-- ============================================================================
-- 3. UPDATE get_breakdown() TO SUPPORT ENTRY/EXIT PAGE DIMENSIONS
-- ============================================================================
...
BEGIN
    CASE p_dimension
        ...
        -- ====================================================================
        -- ENTRY/EXIT PAGE DIMENSIONS (NEW)
        -- ====================================================================
        ...
        ELSE
            RAISE EXCEPTION 'Invalid dimension: %. Must be country, browser, device, referrer, city, region, page, utm_source, utm_medium, utm_campaign, utm_term, utm_content, entry_page, or exit_page', p_dimension;
    END CASE;
END;
...
COMMENT ON COLUMN session.entry_page IS 'First page URL path viewed in this session (landing page)';
COMMENT ON COLUMN session.exit_page IS 'Last page URL path viewed in this session (exit page)';
